...
void Icommutation(double *matrix_part, int proc_size, int rank, int size) {
    ...
    if (rank != size - 1) {
        ...
        MPI_Isend(matrix_part + proc_size * N * N, N * N, MPI_DOUBLE, rank + 1, 42, MPI_COMM_WORLD, &request_arr[2]);
        ////receiving upper layer ( becoming lower layer)
        MPI_Irecv(matrix_part + (proc_size +1) * N * N, N * N, MPI_DOUBLE, rank + 1, 42, MPI_COMM_WORLD, &request_arr[3]);
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        ///lower layer;
        tmp_delta = recalc_layer(matrix_part, aux_matrix_part, start_i, proc_size, delta_x, delta_y, delta_z, constant);
