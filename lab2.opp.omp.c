...
int main(int argc, char **argv) {//задаем потоки
    if(argc != 2){
        printf("wrong param");
        exit(0);
    }
...
#pragma omp parallel for reduction(+:b_length)//делаем копию переменных, тк на параллелных секциях мы постоянно обращаемся к одной и той же переменной
    for (i = 0; i < N; ++i) {
        b_length += b[i] * b[i];
    }
